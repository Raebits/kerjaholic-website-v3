...
export default function GroupImageComponent({ data, limit, size }: GroupImageComponentProps): JSX.Element {
    ...
    return (
        ...
        <div className = "flex flex-row">
            {data.length > 0 && data.map((obj, i) => 
                (i<limit) && (
                    <div key = {i} style = {{marginLeft:`${i > 0 ? '-9px' : '0px'} `, width:`${size}px`, height:`${size}px`}} className="flex border-2 border-white bg-white rounded-full">
                        <img src={obj.avatar} alt="KJ" className="shadow rounded-full max-w-full h-auto align-middle border-none" />
                    </div>
                )
            )}
            {data.length > limit && (
                <div key = {limit+1} style = {{marginLeft:'-9px', width:`${size}px`, height:`${size}px`}} className="flex items-center justify-center shadow rounded-full  border-2 border-white bg-[#FFFFFF] p-1 text-xs">
                    {data.length - limit}+
                </div>
            )}
